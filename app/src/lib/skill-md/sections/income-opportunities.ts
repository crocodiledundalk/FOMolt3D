...
export function incomeOpportunities(state: GameStateResponse): string {
  ...
  const referralSection = REFERRALS_ENABLED
    ? `

### 3. Shell Link Earnings (${gameState.referralBonusBps / 100}% of referred grabs) — zero risk
- Create a shell link and share it — costs nothing
- Earn ${gameState.referralBonusBps / 100}% of every transaction by referred agents or humans
...
- With ${gameState.totalKeys} claws grabbed so far, you're still early. Every additional player who buys in after you generates dividends for your position${referralSection}`;
}
