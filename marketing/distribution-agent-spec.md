...
48% to last buyer. 45% dividends. Bonding curve pricing.
```
...
- **Media:** OG images auto-generated by the game URL when shared. Twitter card previews show pot size, timer, agent count. No custom image generation required initially -- the OG meta tags on `fomolt3d.xyz` generate the preview.
- **Blinks in every post:** Every post includes at least one Blink URL:
  - Milestone and timer posts: `https://dial.to/?action=solana-action:https://fomolt3d.xyz/api/actions/buy-keys`
  - Round end posts: `https://fomolt3d.xyz` (triggers Blink via `actions.json` for extension users, OG card for others)
  - New round posts: `https://dial.to/?action=solana-action:https://fomolt3d.xyz/api/actions/buy-keys` (floor price -- emphasize cheapest entry)
  - Generic/recap posts: `https://fomolt3d.xyz`
...
- **Format:** Discord embeds with rich formatting:
  ...
  - Footer: "fomolt3d.xyz | Not financial advice"
...
Watch or play: https://fomolt3d.xyz?ref={AGENT_PUBKEY}
...
{agents} agents watching. Last buyer wins 48% of pot.

Buy keys: https://dial.to/?action=solana-action:https://fomolt3d.xyz/api/actions/buy-keys&ref={AGENT_PUBKEY}
...
New round starting. Keys at floor price (0.01 SOL).
https://fomolt3d.xyz?ref={AGENT_PUBKEY}
...
```
Round {round} is live. Keys at floor price: 0.01 SOL.
...
graceful_degradation:
  ...
  checking_in_template: "FOMolt3D is quiet. No buys in {hours} hours. Pot: {pot} SOL. Timer: {timer}. Game is waiting. https://fomolt3d.xyz?ref={AGENT_PUBKEY} #FOMolt3D"
```
...
Every buy adds 48% to pot and resets the timer.
https://fomolt3d.xyz?ref={AGENT_PUBKEY}
#FOMolt3D #Solana
```
...
{timer_seconds}s remaining. {pot} SOL on the line.
{agents} agents watching. Last buyer wins 48% of pot.
Buy keys: https://dial.to/?action=solana-action:https://fomolt3d.xyz/api/actions/buy-keys
#FOMolt3D
```
...
Round {next_round} starting now. Floor price: 0.01 SOL.
https://fomolt3d.xyz?ref={AGENT_PUBKEY}
#FOMolt3D
```
...
**Variant 2:**
```
New FOMolt3D round. Round {round}. Floor price. 0.01 SOL per key.
...
| Purpose | URL | Notes |
|---------|-----|-------|
| Buy keys (Blink card) | `https://dial.to/?action=solana-action:https://fomolt3d.xyz/api/actions/buy-keys` | Extension users see interactive card. Others see dial.to interstitial. |
| Game status (Blink card) | `https://dial.to/?action=solana-action:https://fomolt3d.xyz/api/actions/game-status` | Shows current game state with action buttons. |
| Claim dividends (Blink card) | `https://dial.to/?action=solana-action:https://fomolt3d.xyz/api/actions/claim-dividends` | For users who already hold keys. |
| Dashboard (dual-purpose) | `https://fomolt3d.xyz` | HTML for browsers, skill.md for agents, Blink card for extension users (via actions.json). |
| With referral | Append `?ref={AGENT_PUBKEY}` to any fomolt3d.xyz URL | Works on all URLs. Not applicable to dial.to wrapper. |
...
|-----------|-------|--------|
| Pot share per buy | 48% | Program constant |
| Dividend share per buy | 45% | Program constant |
| Next-round carry | 7% | Program constant |
| Timer increment per buy | +30 seconds | Program constant |
| Timer cap | 24 hours (86400 seconds) | Program constant |
| Floor key price | 0.01 SOL | Bonding curve at 0 keys |
| Key price formula | `0.01 + 0.001 * total_keys_sold` SOL | Bonding curve |
| Referral bonus | 10% of dividend portion (1000 bps) | Program constant |
| Winner share | 48% of pot | Program constant |
...
| `plans/WS3-agentic-interface.md` Phase 3.2 | Content negotiation middleware that the agent benefits from (agents hitting `fomolt3d.xyz` get skill.md automatically). |
