...
LiteSVM is a lightweight Solana Virtual Machine that runs directly in your test process. Created by Aursen from Exotic Markets, it eliminates validator overhead while maintaining Solana runtime compatibility.

**Key Advantages:**
- **Performance**: Dramatically faster than `solana-program-test` and `solana-test-validator`
- **Flexibility**: Direct account state manipulation for testing edge cases
...
```rust
// Simulate before executing - doesn't modify state
...
// Enable signature verification (disabled by default for speed)
svm.with_sigverify(true);

// Enable blockhash validation
...
**Key pattern**: Use the exact program ID from your program definition to avoid `ProgramMismatch` errors.

For comprehensive LiteSVM documentation, see the `/litesvm` skill.
